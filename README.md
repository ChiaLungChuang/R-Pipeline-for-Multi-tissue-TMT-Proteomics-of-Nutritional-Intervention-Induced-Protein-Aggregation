...
└── 06_transcriptome-proteome-integration/
    └── 13_transcriptome_proteome_comparison.R
```
...
**13_transcriptome_proteome_comparison.R** — Integrates bulk RNA-seq and TMT proteomics data to compare transcriptome-level and proteome-level fold changes across all five tissues simultaneously. For each tissue, mRNA log2 fold change is plotted against three proteomics metrics: (A) soluble protein fold change, (B) insoluble protein fold change, and (C) insolubility ratio fold change. Genes are color-coded by significance status: grey = not significant in either dataset; blue = RNA-seq only; orange-red = proteomics only; purple = significant in both. R-squared values are computed on all matched genes per tissue per comparison to quantify transcriptome–proteome concordance. Top 10 genes significant in both datasets are labeled per panel, ranked by combined significance (-log10 p RNA-seq + -log10 p proteomics). Produces 15 individual PDFs (5 tissues × 3 comparisons), two combined 3×5 facet PDFs (annotated and clean), 15 matched gene CSVs with full mRNA and proteomics values, and a summary R² table. Gene matching is performed by exact name (RNA-seq `Symbol` vs. proteomics `GN`).
...
| `rnaseq_data.csv` | External | Script 13 |
| `<Tissue>_processed.csv` | Script 07 | Script 13 |
...
Scripts are designed to be run sequentially (01 → 13), though each folder is largely self-contained once the processed CSVs from Script 07 are available.
...
- **Transcriptome–proteome matching** (Script 13) uses exact gene name matching between RNA-seq `Symbol` and proteomics `GN` columns; when a gene appears multiple times in the proteomics data, the entry with the lowest `p_value_ratio` is retained.
